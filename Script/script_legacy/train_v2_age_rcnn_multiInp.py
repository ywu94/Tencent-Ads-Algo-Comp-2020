...
from clf_rcnn import Multi_Seq_RCNN_Classifier
...
def train(model, task, y_list, x_list, checkpoint_dir, checkpoint_prefix, device, batch_size=512, max_seq_len=100, lr=1e-3, resume_surfix=None, logger=None):
	...
	for epoch, file_idx_list in task:
		...
		if logger:
			logger.info('Epoch {}/{} Done - Test Loss: {:.6f}, Accuracy: {:.6f}'.format(
				epoch, task[-1][0], test_running_loss/test_n_batch, acc))
...
if __name__=='__main__':
	...
	task_name = 'train_v2_multi_rcnn_multiInp'
	...
	model = Multi_Seq_RCNN_Classifier(10, [128, 128, 128, 128], [128, 128, 128, 128], conv_channel=192, max_seq_len=max_seq_len)
